...
namespace XoSoKienThiet.Controllers.Core
{
  ...
  //  [Authorize]
    public class MenuController : DefaultReposityController<Menu>
    {
        ...
        public MenuController(DataContext context, IMenuService service) : base(context, NameCollection)
        {
            _service = service;
       ...
       [HttpPost]
        ...
        [HttpGet]
        [Route("getTreeListNoiBo")]
        public async Task<IActionResult> GetTreeListNoiBo()
        {
            try
            {
                var response = await _service.GetTreeListNoiBo();
                return Ok(new ResultMessageResponse()
                    .WithData(response)
                    .WithCode(DefaultCode.SUCCESS)
                    .WithMessage(DefaultMessage.GET_DATA_SUCCESS));
        ...
        [HttpGet]
        [Route("getTreeMenuAndAction")]
        public async Task<IActionResult> GetTreeMenuAndAction()
        {
            try
            {
                var response = await _service.GetTreeListAndAction();
                return Ok(new ResultMessageResponse()
                    .WithData(response)
                    .WithCode(DefaultCode.SUCCESS)
                    .WithMessage(DefaultMessage.GET_DATA_SUCCESS));
