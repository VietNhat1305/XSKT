...
export default {
  ...
  created() {
    const user = {
      ...
      permissions: [
        'manage-SUKIEN-65558d1a4a780d6d93a2840e', 
        'create-SUKIEN-65558d1a4a780d6d93a2840e',
        'update-SUKIEN-65558d1a4a780d6d93a2840e',
        'delete-SUKIEN-65558d1a4a780d6d93a2840e'
      ]
    }
  },
  methods: {
    ...
    async handleDelete() {
      if (this.model._id != 0 && this.model._id != null && this.model._id) {
        await this.$store.dispatch("suKienStore/delete",{_id :  this.model._id}).then((res) => {
          if (res.code===0) {
            this.fnGetList();
            this.showDeleteModal = false;
            this.model=suKienModel.baseJson()
          }
          var a = {
            message: res.message,
            code: res.code
    ...
    async handleSubmit(e) {
      ...
      if (this.$v.$invalid) {
        ...
        if (
          ...
          // Update model
          await this.$store.dispatch("suKienStore/update", this.model).then((res) => {
            if (res.code === 0) {
          ...
          await this.$store.dispatch("suKienStore/create", this.model).then((res) => {
            if (res.code === 0) {
              this.fnGetList();
              this.showModal = false;
              this.model={}
    ...
    async handleUpdate(id) {
      await this.$store.dispatch("suKienStore/getById", {_id : id}).then((res) => {
        if (res.code===0) {
          console.log(res)
          this.model = suKienModel.toJson(res.data);
          this.showModal = true;
        } else {
          this.$store.dispatch("snackBarStore/addNotify", {
            message: res.message,
    ...
    myProvider (ctx) {
      ...
      this.loading = true
      try {
        let promise =  this.$store.dispatch("suKienStore/getPagingParams", params)
  ...
  watch: {
    ...
    showModal(status) {
      if (status == false) this.model = suKienModel.baseJson();
    },
    // model() {
    //   return this.model;
    // },
            ...
            <div class="row mb-2">
              ...
              <div class="col-sm-8">
                <div class="text-sm-end">
                  <b-button
                      v-if="$can('manage-SUKIEN-65558d1a4a780d6d93a2840e') || $can('create-SUKIEN-65558d1a4a780d6d93a2840e')"
                      type="button"
                      class="btn cs-btn-primary btn-rounded mb-2 me-2"
                      size="sm"
                      @click="showModal = true"
                  >
                    <i class="mdi mdi-plus me-1"></i> Thêm sự kiện
                  </b-button>
                  <b-modal
                      v-model="showModal"
                  ...
                  >
                    ...
                    >
                      <div class="row">
                        <div class="col-12">
                          <div class="mb-3">
                            <label class="text-left">Nội dung sự kiện </label>
                ...
                <div class="table-responsive mb-0">
                  ...
                  >
                    ...
                    <template v-slot:cell(process)="data">
                      <button
                          v-if="$can('manage-SUKIEN-65558d1a4a780d6d93a2840e') || $can('update-SUKIEN-65558d1a4a780d6d93a2840e')"
                          type="button"
                          size="sm"
                      ...
                      <button
                          v-if="$can('manage-SUKIEN-65558d1a4a780d6d93a2840e') || $can('delete-SUKIEN-65558d1a4a780d6d93a2840e')"
                          type="button"
                          size="sm"
                          class="btn btn-outline btn-sm"
