...
export default {
  ...
  data() {
    return {
      ...
      submitted: false,
      modelSlider: linkModel.baseJson(),
      listRole: [],
  ...
  created() {
    const user = {
      ...
      permissions: [
        'manage-BANNERLIENKET-6556df302b5510055e04df02', 
        'create-BANNERLIENKET-6556df302b5510055e04df02',
        'update-BANNERLIENKET-6556df302b5510055e04df02',
        'delete-BANNERLIENKET-6556df302b5510055e04df02'
      ]
    }
  },
  watch: {
    showModalSlider(status) {
      if (status == false) this.modelSlider = linkModel.baseJson() ;
    },
    showDeleteModal(val) {
      if (val == false) {
        this.model.id = null;
  ...
  methods: {
      ...
      if (this.$v.$invalid) {
        ...
        if (
          ...
          if (this.modelSlider == null || (this.modelSlider != null && this.modelSlider.file == null)) {
            ...
            await this.$store.dispatch("linkStore/update", this.modelSlider).then((res) => {
              if (res.code === 0) {
                this.showModalSlider = false;
                this.modelSlider=linkModel.baseJson()
          ...
          if (this.modelSlider == null || (this.modelSlider != null && this.modelSlider.file == null)) {
            ...
            await this.$store.dispatch("linkStore/create", this.modelSlider).then((res) => {
              if (res.code === 0) {
                this.fnGetList();
                this.showModalSlider = false;
                this.model={}
    ...
    async handleDelete() {
      if (this.modelSlider._id != 0 && this.modelSlider._id != null && this.modelSlider._id) {
        await this.$store.dispatch("linkStore/delete",{_id :  this.modelSlider._id}).then((res) => {
          if (res.code===0) {
            this.fnGetList();
    ...
    async handleUpdate(id) {
      await this.$store.dispatch("linkStore/getById", {_id : id}).then((res) => {
        if (res.code===0) {
          console.log(res)
          this.modelSlider = linkModel.toJson(res.data);
    ...
    myProvider (ctx) {
      ...
      this.loading = true
      try {
        let promise =  this.$store.dispatch("linkStore/getPagingParams", params)
            ...
            <div class="row mb-2">
              ...
              <div class="col-sm-8">
                <div class="text-sm-end">
                  <b-button
                      v-if="$can('manage-BANNERLIENKET-6556df302b5510055e04df02') || $can('create-BANNERLIENKET-6556df302b5510055e04df02')"
                      type="button"
                      class="btn cs-btn-primary btn-rounded mb-2 me-2"
                      size="sm"
                      @click="showModalSlider = true"
                  >
                    <i class="mdi mdi-plus me-1"></i> Thêm banner liên kết
                  </b-button>
                  <b-modal
                      v-model = "showModalSlider"
                      title="Cập nhật banner liên kết"
                  ...
                  >
                    ...
                    >
                      <div class="row">
                        <div class="col-12">
                          <div class="mb-2">
                            <label class="text-left mb-0">Đường dẫn liên kết</label>
                            <span style="color: red">&nbsp;*</span>
                            <input
                                v-model="modelSlider.link"
                                id="percent"
                                type="link"
                                class="form-control"
                                :class="{
                      'is-invalid':
                        submitted && $v.modelSlider.link.$error,
                    }"
                            />
                            <div
                                v-if="submitted && !$v.modelSlider.link.required"
                                class="invalid-feedback"
                            >
                              Đường dẫn liên kết không được trống.
                            </div>
                          </div>
                        </div>
                        <div class="col-12" >
                          <label class="text-left">Chọn ảnh (Kích thước 500 x 200) </label>
                ...
                <div class="table-responsive mb-0">
                  ...
                  >
                    ...
                    <template v-slot:cell(process)="data">
                      <button
                          v-if="$can('manage-BANNERLIENKET-6556df302b5510055e04df02') || $can('update-BANNERLIENKET-6556df302b5510055e04df02')"
                          type="button"
                          size="sm"
                      ...
                      <button
                          v-if="$can('manage-BANNERLIENKET-6556df302b5510055e04df02') || $can('delete-BANNERLIENKET-6556df302b5510055e04df02')"
                          type="button"
                          size="sm"
                          class="btn btn-outline btn-sm"
